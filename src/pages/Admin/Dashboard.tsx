...
type Tab = 'overview' | 'posts' | 'projects' | 'settings';
...
function Dashboard({ token, onLogout }: DashboardProps) {
    ...
    const [overviewData, setOverviewData] = useState<{ posts: ContentItem[]; projects: ContentItem[] }>({ posts: [], projects: [] });
    ...
    const fetchItems = React.useCallback(async (type: Tab) => {
        setLoading(true);
        setItems([]);
        ...
        if (type === 'overview') {
            try {
                const [postsRes, projectsRes] = await Promise.all([
                    octokit.rest.repos.getContent({ owner: REPO_OWNER, repo: REPO_NAME, path: 'src/content/posts', ref: BRANCH }).catch(() => ({ data: [] as ContentItem[] })),
                    octokit.rest.repos.getContent({ owner: REPO_OWNER, repo: REPO_NAME, path: 'src/content/projects', ref: BRANCH }).catch(() => ({ data: [] as ContentItem[] }))
                ]);
        ...
        if (type === 'settings') {
            try {
                ...
                setEditingItem({
                    ...
                    sha: file.sha,
                    isNew: false,
                    frontmatter: configData as Record<string, any>,
                    body: ''
                });
            } catch (err) {
    ...
    const loadItem = async (item: ContentItem) => {
        setLoading(true);
        try {
            const { data } = await octokit.rest.repos.getContent({
                owner: REPO_OWNER,
    ...
    const handleCreateNew = (targetTab: Tab = activeTab) => {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (activeTab === 'settings') {
                ...
                setEditingItem(prev => prev ? ({ ...prev, sha: response.data.content?.sha ?? prev.sha }) : prev);
            } else {
                // Go back to list
                setEditingItem(null);
            }
    ...
    const updateFrontmatter = (key: string, value: string | string[]) => {
        setEditingItem(prev => {
            if (!prev) return prev;
            const updated: EditingItem = { ...prev, frontmatter: { ...prev.frontmatter, [key]: value } };

            // Auto-generate slug and path for new items based on title
            if (prev.isNew && (key === 'title' || key === 'date')) {
                const title = String(updated.frontmatter.title || 'new-entry');

                // Convert turkish chars to english equivalents and generate slug
                const charMap: Record<string, string> = { 'ğ':'g', 'ü':'u', 'ş':'s', 'ı':'i', 'ö':'o', 'ç':'c', 'Ğ':'G', 'Ü':'U', 'Ş':'S', 'İ':'I', 'Ö':'O', 'Ç':'C' };
